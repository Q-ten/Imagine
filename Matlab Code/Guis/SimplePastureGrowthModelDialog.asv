function varargout = SimplePastureGrowthModelDialog(varargin)
% SIMPLEPASTUREGROWTHMODELDIALOG M-file for SimplePastureGrowthModelDialog.fig
%      SIMPLEPASTUREGROWTHMODELDIALOG, by itself, creates a new SIMPLEPASTUREGROWTHMODELDIALOG or raises the existing
%      singleton*.
%
%      H = SIMPLEPASTUREGROWTHMODELDIALOG returns the handle to a new SIMPLEPASTUREGROWTHMODELDIALOG or the handle to
%      the existing singleton*.
%
%      SIMPLEPASTUREGROWTHMODELDIALOG('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in SIMPLEPASTUREGROWTHMODELDIALOG.M with the given input arguments.
%
%      SIMPLEPASTUREGROWTHMODELDIALOG('Property','Value',...) creates a new SIMPLEPASTUREGROWTHMODELDIALOG or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before SimplePastureGrowthModelDialog_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to SimplePastureGrowthModelDialog_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help SimplePastureGrowthModelDialog

% Last Modified by GUIDE v2.5 13-Aug-2012 09:28:57

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @SimplePastureGrowthModelDialog_OpeningFcn, ...
                   'gui_OutputFcn',  @SimplePastureGrowthModelDialog_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
function gm = setupParameters(varargin)
    ...
    default.FOO.firstFeedGapMonth = 3;
...
% --- Executes just before SimplePastureGrowthModelDialog is made visible.
function SimplePastureGrowthModelDialog_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to SimplePastureGrowthModelDialog (see VARARGIN)
...
% Choose default command line output for SimplePastureGrowthModelDialog
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes SimplePastureGrowthModelDialog wait for user response (see UIRESUME)
uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = SimplePastureGrowthModelDialog_OutputFcn(hObject, eventdata, handles) 
...
function editFodderPrice_Callback(hObject, eventdata, handles)
% hObject    handle to editFodderPrice (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of editFodderPrice as text
%        str2double(get(hObject,'String')) returns contents of editFodderPrice as a double
val = str2double(get(handles.editFodderPrice, 'String'));

if isnan(val)
    set(hObject, 'String', num2str(handles.fodderCosts.pricePerTonne));
else
    handles.fodderCosts.pricePerTonne = val;
    guidata(hObject, handles);
end

% --- Executes during object creation, after setting all properties.
function editFodderPrice_CreateFcn(hObject, eventdata, handles)
% hObject    handle to editFodderPrice (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
...
set(handles.popupmenuStartMonth, 'Value', handles.FOO.firstFeedGapMonth);
...
model.FOO.firstFeedGapMonth = get(handles.popupmenuStartMonth, 'Value');
...
cropInfo.cropYieldUnits = 't / Ha';
sis = SpatialInteractionsDialog(cropInfo, handles.spatialInteractions);
if ~isempty(sis)
